...
interface OptionsSelectorProps {
  onSelectToken: (token: OptionsUnderlying) => void;
  selectedSymbol?: string;
}

export default function OptionsSelector({ onSelectToken, selectedSymbol }: OptionsSelectorProps) {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: "symbol" | "price" | "change" | "marketCap"; direction: "asc" | "desc" }>({
    key: "marketCap",
    direction: "desc",
  });
  ...
  const handleSort = (key: "symbol" | "price" | "change" | "marketCap") => {
  ...
  const getSortIcon = (key: "symbol" | "price" | "change" | "marketCap") => {
  ...
  if (perpsLoading || priceLoading) {
    return (
      <div className={styles.loadingContainer}>
        ...
        <div className={styles.loadingText}>Loading options markets...</div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.tokenSelector}>
      <div className={styles.header}>
        <h2 className={styles.title}>Base Options Markets</h2>
        <div className={styles.stats}>
          <span className={styles.statItem}>
            <span className={styles.statLabel}>Underlyings</span>
            <span className={styles.statValue}>{activeTokens.length}</span>
          </span>
          <span className={styles.statItem}>
            <span className={styles.statLabel}>Expiries</span>
      ...
      <div className={styles.searchContainer}>
        ...
        <input
          type="text"
          placeholder="Search option underlyings..."
          value={searchTerm}
          ...
          <thead>
            <tr>
              <th onClick={() => handleSort("symbol")} className={styles.sortable}>
                <span className={styles.headerLabelWrap}>Underlying {getSortIcon("symbol")}</span>
              </th>
      ...
      <div className={styles.footer}>
        <span>Showing {filteredAndSortedTokens.length} option underlyings on Base</span>
      </div>
    </div>
  );
}
