...
const emaData = (candles: SanitizedCandle[], period: number): Array<{ time: Time; value: number }> => {
...
const sanitizeCandles = (candles: Candlestick[]): SanitizedCandle[] => {
  const mapped: Array<SanitizedCandle | null> = candles
    .map((candle) => {
      ...
      if (time === null || !Number.isFinite(time) || !isValidPrice) {
        return null;
      }
...
export function UnifiedPriceChart({ 
  ...
  const normalizedTimeframes = useMemo(
    () => timeframeOptions.map((item) => (typeof item === "string" ? { value: item, label: item } : item)),
    [timeframeOptions]
  );

  const sanitizedCandles = useMemo(() => sanitizeCandles(candles ?? []), [candles]);
  const ema9 = useMemo(() => emaData(sanitizedCandles, 9), [sanitizedCandles]);
  const ema21 = useMemo(() => emaData(sanitizedCandles, 21), [sanitizedCandles]);
  const ema50 = useMemo(() => emaData(sanitizedCandles, 50), [sanitizedCandles]);
  ...
  const adjustZoom = (multiplier: number) => {
    ...
    const nextSpacing = Math.max(2, Math.min(60, currentSpacing * multiplier));
  ...
  }, [height, palette.chartBackground, palette.chartText, palette.grid]);
  ...
  }, [sanitizedCandles, ema9, ema21, ema50]);
    ...
    <div style={{ border: `1px solid ${palette.buttonBorder}`, borderRadius: 14, overflow: "hidden", background: palette.chartBackground }}>
      <div style={{ height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "0 12px", borderBottom: `1px solid ${palette.buttonBorder}`, background: palette.headerSurface }}>
            ...
            {normalizedTimeframes.map((timeframe) => (
              <button
                ...
                style={{
                  ...
                  boxShadow: selectedTimeframe === timeframe.value ? `inset 0 0 0 1px ${palette.accent}22` : "none",
                }}
              >
                {timeframe.label}
              </button>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, position: "relative" }}>
          <div style={{ position: "relative" }}>
            <button
              type="button"
              onClick={() => setShowChartTypeMenu((value) => !value)}
              style={{
          ...
          <button
            type="button"
            onClick={() => setShowIndicatorsMenu((value) => !value)}
            style={{
              border: `1px solid ${palette.buttonBorder}`,
              borderRadius: 8,
              padding: "6px 10px",
          ...
          {showIndicatorsMenu ? (
            <div style={{ position: "absolute", top: 42, right: 0, zIndex: 6, background: palette.chartBackground, border: `1px solid ${palette.buttonBorder}`, borderRadius: 10, minWidth: 170, padding: 8, boxShadow: theme === "dark" ? "0 8px 24px rgba(0,0,0,0.35)" : "0 8px 24px rgba(15,23,42,0.12)" }}>
              <button type="button" onClick={() => setShowEma9((value) => !value)} style={{ width: "100%", border: "none", background: "transparent", color: palette.headerText, textAlign: "left", padding: "7px 8px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                {showEma9 ? "☑" : "☐"} EMA 9 {latest(ema9) ? `(${latest(ema9)?.toFixed(2)})` : ""}
              </button>
              <button type="button" onClick={() => setShowEma21((value) => !value)} style={{ width: "100%", border: "none", background: "transparent", color: palette.headerText, textAlign: "left", padding: "7px 8px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                {showEma21 ? "☑" : "☐"} EMA 21 {latest(ema21) ? `(${latest(ema21)?.toFixed(2)})` : ""}
              </button>
              <button type="button" onClick={() => setShowEma50((value) => !value)} style={{ width: "100%", border: "none", background: "transparent", color: palette.headerText, textAlign: "left", padding: "7px 8px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                {showEma50 ? "☑" : "☐"} EMA 50 {latest(ema50) ? `(${latest(ema50)?.toFixed(2)})` : ""}
              </button>
              <button type="button" onClick={() => setShowVolume((value) => !value)} style={{ width: "100%", border: "none", background: "transparent", color: palette.headerText, textAlign: "left", padding: "7px 8px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                {showVolume ? "☑" : "☐"} Volume
              </button>
            </div>
          ) : null}
      ...
      <div style={{ display: "flex", width: "100%", height }}>
        <div style={{ width: 50, borderRight: `1px solid ${palette.buttonBorder}`, display: "flex", flexDirection: "column", alignItems: "center", padding: "8px 6px", gap: 6, background: palette.railSurface }}>
          <button type="button" onClick={() => setActiveTool("pointer")} title="Pointer / Edit" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: activeTool === "pointer" ? palette.activeButtonBg : "transparent", color: palette.headerText, cursor: "pointer", fontSize: 16 }}>
            ↖
          </button>
          <button type="button" onClick={() => setActiveTool("trendline")} title="Trend Line" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: activeTool === "trendline" ? palette.activeButtonBg : "transparent", color: palette.headerText, cursor: "pointer", display: "grid", placeItems: "center" }}>
            <DrawToolIcon color={palette.headerText} />
          </button>
          <button type="button" onClick={() => setActiveTool("fib")} title="Fib Retracement" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: activeTool === "fib" ? palette.activeButtonBg : "transparent", color: palette.headerText, cursor: "pointer", display: "grid", placeItems: "center" }}>
            <FibToolIcon color={palette.headerText} />
          </button>
          <button type="button" onClick={() => setActiveTool("long")} title="Long Position" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: activeTool === "long" ? palette.activeButtonBg : "transparent", cursor: "pointer", display: "grid", placeItems: "center" }}>
            <PositionIcon type="long" />
          </button>
          <button type="button" onClick={() => setActiveTool("short")} title="Short Position" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: activeTool === "short" ? palette.activeButtonBg : "transparent", cursor: "pointer", display: "grid", placeItems: "center" }}>
            <PositionIcon type="short" />
          </button>
          <div style={{ width: "100%", height: 1, background: palette.buttonBorder, margin: "4px 0" }} />
          <button type="button" onClick={() => adjustZoom(1.2)} title="Zoom In" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: "transparent", color: palette.headerText, cursor: "pointer", fontSize: 16 }}>
            +
          </button>
          <button type="button" onClick={() => adjustZoom(1 / 1.2)} title="Zoom Out" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: "transparent", color: palette.headerText, cursor: "pointer", fontSize: 16 }}>
            −
          </button>
          <button type="button" onClick={() => chartRef.current?.timeScale().fitContent()} title="Reset View" style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${palette.buttonBorder}`, background: "transparent", color: palette.headerText, cursor: "pointer", fontSize: 16 }}>
            ⟲
          </button>
          <button
            type="button"
        ...
        <div style={{ position: "relative", flex: 1, height: "100%" }}>
            ...
            <svg width="100%" height="100%" style={{ overflow: "visible" }}>
              {drawings.map((drawing) => {
                if (drawing.kind === "trendline") {
                  ...
                  return (
                    <g key={drawing.id}>
                      <line x1={start.x} y1={start.y} x2={end.x} y2={end.y} stroke="#60a5fa" strokeWidth={selectedDrawingId === drawing.id ? 2.6 : 2} />
          ...
          {isLoading && !sanitizedCandles.length ? (
