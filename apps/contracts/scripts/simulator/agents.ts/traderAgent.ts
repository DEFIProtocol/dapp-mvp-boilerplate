// agents/traderAgents.ts
import { BaseAgent, Position } from '../agents/baseAgent.js';
import { Order } from '../core/orderGenerator.js';
    ...
    if (Math.abs(momentum) > this.momentumThreshold) {
      ...
      if (existingDirection < 2) { // Limit positions per direction
        const size = this.random.range(
          Number(ethers.formatUnits(this.config.behavior.minTradeSize, 6)),
          Number(ethers.formatUnits(this.config.behavior.maxTradeSize, 6))
        );
...
export class RetailTraderAgent extends BaseAgent {
  async act(currentPrice: number, marketTrend?: 'up' | 'down' | 'neutral'): Promise<void> {
    ...
    const minSize = Number(ethers.formatUnits(this.config.behavior.minTradeSize, 6));
    const maxSize = Number(ethers.formatUnits(this.config.behavior.maxTradeSize, 6));
    ...
    if (this.positions.length > 0) {
      ...
      if (this.random.next() > 0.3) {
        ...
        const additionalSize = this.random.range(
          Number(ethers.formatUnits(this.config.behavior.minTradeSize, 6)) * 0.5,
          Number(ethers.formatUnits(this.config.behavior.maxTradeSize, 6)) * 0.3
        );
      ...
      const size = this.random.range(
        Number(ethers.formatUnits(this.config.behavior.minTradeSize, 6)) * 0.8,
        Number(ethers.formatUnits(this.config.behavior.maxTradeSize, 6))
      );
