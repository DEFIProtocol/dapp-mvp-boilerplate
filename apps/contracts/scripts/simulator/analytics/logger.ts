...
export class SimulationLogger {
  ...
  private appendToCsv(step: number, metrics: ProtocolMetrics): void {
    ...
    const line = [
      ...
      formatUnits(metrics.adlRequestedNotional, 6),
      formatUnits(metrics.adlCoveredNotional, 6),
      formatUnits(metrics.adlRemainingDeficit, 6),
      metrics.adlEvents,
      metrics.proactiveAdlEvents,
      metrics.proactiveAdlSoftEvents,
      metrics.proactiveAdlHardEvents,
      metrics.stepAdlEvents,
      metrics.stepProactiveAdlEvents,
  ...
  private formatSummaryText(summary: any): string {
    return [
      ...
      '',
      'ADL FLOW',
      `ADL Events: ${summary.adl.events}`,
      `Proactive ADL Events: ${summary.adl.proactiveEvents}`,
      `Proactive Soft Triggers: ${summary.adl.proactiveSoftEvents}`,
      `Proactive Hard Triggers: ${summary.adl.proactiveHardEvents}`,
      `ADL Requested Notional: $${summary.adl.requestedNotional}`,
      `ADL Covered Notional: $${summary.adl.coveredNotional}`,
      `ADL Remaining Deficit: $${summary.adl.remainingDeficit}`,
      '',
    ].join('\n');
  }
  ...
  private saveLiquidatorActivityTable(): void {
    ...
    const lines = this.logs.metrics.map((m, idx) => [
      ...
      formatUnits(m.adlRequestedNotional, 6),
      formatUnits(m.adlCoveredNotional, 6),
      formatUnits(m.adlRemainingDeficit, 6),
      m.adlEvents,
      m.proactiveAdlEvents,
      m.proactiveAdlSoftEvents,
      m.proactiveAdlHardEvents,
      m.stepAdlEvents,
      m.stepProactiveAdlEvents,
    ].join(','));
