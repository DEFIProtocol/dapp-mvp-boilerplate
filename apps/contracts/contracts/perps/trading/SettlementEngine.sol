...
interface ISettlementMarkPriceDiagnostics {
...
contract SettlementEngine is EIP712 {
    ...
    function settleMatch(
        ...
        bytes32 marketId = _resolveMarketId(longOrder.marketId);
    ...
    function settleMatches(
        ...
        for (uint256 i = 0; i < n; i++) {
            bytes32 marketId = _resolveMarketId(longOrders[i].marketId);
            _maybeAutoPauseOnOracleStale(marketId);
            uint256 matchFees;
    ...
    function closePositionViaMatch(
        ...
        uint256 takerSubAccountId = _selectTradingSubAccountId(msg.sender);
        uint256 makerSubAccountId = _selectTradingSubAccountId(counterOrder.trader);

        _requireAvailableCollateral(msg.sender, takerSubAccountId, takerFee);
        _requireAvailableCollateral(counterOrder.trader, makerSubAccountId, _calculateRequiredMargin(matchSize) + makerFee);

        _chargeFeesAndMaybeRoute(
            msg.sender,
            takerSubAccountId,
            false,
            counterOrder.trader,
        ...
        uint256 nonce = perpStorage.incrementFillNonce();
        matchId = keccak256(abi.encodePacked(positionId, counterHash, nonce));
    ...
    function liquidatePositionViaMatch(uint256 positionId, uint256 matchSize) external notPaused returns (bytes32 matchId) {
        ...
        uint256 liquidatorSubAccountId = _selectTradingSubAccountId(msg.sender);
        uint256 liquidatedSubAccountId = position.subAccountId;

        _requireAvailableCollateral(msg.sender, liquidatorSubAccountId, _calculateRequiredMargin(matchSize) + liquidatorFee);
        _requireAvailableCollateral(position.trader, liquidatedSubAccountId, liquidatedMakerFee);
        ...
        uint256 nonce = perpStorage.incrementFillNonce();
        matchId = keccak256(abi.encodePacked(positionId, msg.sender, nonce));
    ...
    function _settleMatch(
        ...
        
        {
            ...
            // Create unique match ID using fillNonce for replay protection
            matchId = _nextMatchId(longHash, shortHash);
        }
        ...
        uint256 requiredMargin = _calculateRequiredMargin(matchSize);

        uint256 longSubAccountId = _selectTradingSubAccountId(longOrder.trader);
        uint256 shortSubAccountId = _selectTradingSubAccountId(shortOrder.trader);
        
        // Check available collateral
        _requireAvailableCollateral(longOrder.trader, longSubAccountId, requiredMargin + longFee);
        _requireAvailableCollateral(shortOrder.trader, shortSubAccountId, requiredMargin + shortFee);
        ...
        uint256 leverage = matchSize / requiredMargin;
    ...
    function _nextMatchId(bytes32 left, bytes32 right) internal returns (bytes32) {
        uint256 nonce = perpStorage.incrementFillNonce();
        return keccak256(abi.encodePacked(left, right, nonce));
    }

    function _chargeFeesAndMaybeRoute(
        address firstTrader,
        uint256 firstSubAccountId,
        bool firstIsMaker,
        address secondTrader,
        uint256 secondSubAccountId,
        bool secondIsMaker,
        uint256 matchSize,
        bytes32 marketId,
        bool routeFeesNow
    ) internal returns (uint256 totalFees) {
        uint256 firstCharge = collateralManager.chargeTradingFeesForSubAccount(firstTrader, firstSubAccountId, matchSize, firstIsMaker, marketId);
        uint256 secondCharge = collateralManager.chargeTradingFeesForSubAccount(secondTrader, secondSubAccountId, matchSize, secondIsMaker, marketId);
        totalFees = firstCharge + secondCharge;

        bool bothLegacy =
            firstSubAccountId == perpStorage.LEGACY_SUBACCOUNT_ID() &&
            secondSubAccountId == perpStorage.LEGACY_SUBACCOUNT_ID();

        if (routeFeesNow && totalFees > 0 && bothLegacy) {
    ...
    function _openMatchedPositions(
        ...
        positionManager.openPositionWithMarketAndSubAccount(
            firstTrader,
            firstSide,
            matchSize,
            firstLeverage,
    ...
    function _requireMarketTradeable(bytes32 marketId) internal view {
        ...
        require(!perpStorage.marketOracleStalePaused(marketId), "Market oracle stale paused");
    }
    ...
    function _maybeAutoPauseOnOracleStale(bytes32 marketId) internal {
        ...
        bool stale = _isOracleStale(market.feedId);
        if (stale) {
            perpStorage.setMarketOracleStalePause(resolvedMarketId, true);
        }
    }

    function _isOracleStale(bytes32 feedId) internal view returns (bool) {
        try ISettlementMarkPriceDiagnostics(perpStorage.markOracle()).getPriceBreakdown(feedId) returns (
            uint256,
            uint256,
            uint256,
            uint256,
    ...
    function _selectTradingSubAccountId(address trader) internal view returns (uint256) {
        if (perpStorage.hasDefaultSubAccount(trader)) {
            return perpStorage.defaultSubAccountId(trader);
        }

        return perpStorage.LEGACY_SUBACCOUNT_ID();
    }

    function _requireAvailableCollateral(address trader, uint256 subAccountId, uint256 required) internal {
        _maybePullJitCollateralShortfall(trader, subAccountId, required);
    ...
    function _maybePullJitCollateralShortfall(address trader, uint256 subAccountId, uint256 required) internal {
        ...
        uint256 available;
        if (subAccountId == perpStorage.LEGACY_SUBACCOUNT_ID()) {
            available = collateralManager.getAvailableCollateral(trader);
        } else {
            available = collateralManager.getAvailableCollateralForSubAccount(trader, subAccountId);
        }
