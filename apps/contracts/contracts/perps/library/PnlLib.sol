...
library PnlLib {
    ...
    // Calculate bankruptcy price (equity == 0, excludes maintenance buffer)
    function calculateBankruptcyPrice(
        Position memory position,
        uint256 margin
    ) internal pure returns (uint256 bankPrice) {
        require(position.exposure > 0, "Zero exposure");

        if (position.side == Side.Long) {
            uint256 priceDrop = (margin * position.entryPrice) / position.exposure;
            bankPrice = position.entryPrice > priceDrop ? position.entryPrice - priceDrop : 0;
        } else {
            uint256 priceIncrease = (margin * position.entryPrice) / position.exposure;
            bankPrice = position.entryPrice + priceIncrease;
        }
    }
